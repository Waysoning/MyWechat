package com.example.example1.mywechat.bluetooth;
...
import com.example.example1.mywechat.settingsFragment;
...
public class chatService {
    ...
    private synchronized void setState(int state) {
        mState = state;
        mHandler.obtainMessage(settingsFragment.MESSAGE_STATE_CHANGE, state, -1).sendToTarget();
    }
    ...
    public synchronized void connected(BluetoothSocket socket, BluetoothDevice device) {
        ...
        Message msg = mHandler.obtainMessage(settingsFragment.MESSAGE_DEVICE_NAME);
        Bundle bundle = new Bundle();
        bundle.putString(settingsFragment.DEVICE_NAME, device.getName());
        msg.setData(bundle);
    ...
    private void connectionFailed() {
        setState(STATE_LISTEN);
        Message msg = mHandler.obtainMessage(settingsFragment.MESSAGE_TOAST);
        Bundle bundle = new Bundle();
        bundle.putString(settingsFragment.TOAST, "链接不到设备");
        msg.setData(bundle);
        mHandler.sendMessage(msg);
    }
    ...
    private void connectionLost() {
        ...
        Message msg = mHandler.obtainMessage(settingsFragment.MESSAGE_TOAST);
        Bundle bundle = new Bundle();
        bundle.putString(settingsFragment.TOAST, "设备链接中断");
        msg.setData(bundle);
        mHandler.sendMessage(msg);
    }
    ...
    private class ConnectedThread extends Thread {
        ...
        public void run() {
            ...
            while (true) {
                try {
                    ...
                    mHandler.obtainMessage(settingsFragment.MESSAGE_READ, bytes, -1, buffer).sendToTarget();
                } catch (IOException e) {
                    connectionLost();
                    break;
                }
        ...
        public void write(byte[] buffer) {
            try {
                ...
                mHandler.obtainMessage(settingsFragment.MESSAGE_WRITE, -1, -1, buffer).sendToTarget();
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
